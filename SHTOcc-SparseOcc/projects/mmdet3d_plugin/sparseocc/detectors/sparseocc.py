...
class SparseOcc(BEVDepth):
    def __init__(self, loss_norm=False, **kwargs):
    ...
    def forward_test(self,
        ...
        # print(self.img_view_transformer.nx)
        # torch.cuda.synchronize()
        # t0 = time.time()
    ...
    def forward_dummy(self,
        ...
        voxel_feats, img_feats, depth = self.extract_feat(points=None, img=img_inputs, img_metas=img_metas)
    ...
    def simple_test(self, img_metas, img=None, rescale=False, points_occ=None, gt_occ=None, points_uv=None):
        ...
        print('decoder=',time3-time2)
        print('whole=',time3-time1)
        torch.cuda.synchronize()
        t1 = time.time()
        ...
        output_voxels_refine = None
        if output['output_voxel_refine'] is not None:
            output_voxels_refine = output['output_voxel_refine'][0]
        ...
        output['output_voxel_refine'] = output_voxels_refine
        ...
        # D = 32
        # ratio = 1
        # target_voxels = gt_occ.reshape(1, H, ratio, W, ratio, D, ratio).permute(0,1,3,5,2,4,6).reshape(1, H, W, D, ratio**3)
        ...
        # nonempty_coords = target_voxels.nonzero()
        # nonempty_coords = nonempty_coords.type(torch.long).to(target_voxels.device)
        # self.nonempty_num = self.nonempty_num + nonempty_coords.shape[0]
        # self.sample_num = self.sample_num + 1
        # print(self.nonempty_num)
        # print(self.sample_num)
        # print(self.nonempty_num / self.sample_num / W / H / D)
...
@DETECTORS.register_module()
class OccupancyFormer4D(SparseOcc):
