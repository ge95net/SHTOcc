...
class Mask2FormerNuscOccHead(MaskFormerHead):
    ...
    def __init__(self,
        ...
        #self.TailVoxelSampler = TailVoxelSampler()
        ...
        #         in_channel=feat_channels + self.num_classes + 1, embed_dims=feat_channels, num_classes=self.num_classes + 1
        #     )
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list,
                    gt_masks_list, gt_lidarseg_list, img_metas,tail_class=[2,5,7,8,10,12,14]):
        ...
        tail_class_tensor = torch.tensor(tail_class, device=labels.device)

        # 生成布尔掩码（True表示属于tail_class）
        mask = torch.isin(labels, tail_class_tensor)

        # 检查是否存在符合条件的元素
        has_tail = mask.any()  # 如果有至少一个True返回True
        ...
        if has_tail:
            # 提取所有符合条件的索引
            tail_indices = torch.nonzero(mask, as_tuple=True)[0]
          
            
            # 同时可以提取对应的标签值
            tail_labels = labels[tail_indices]
            tail_cls_score = cls_scores[tail_indices]
            tail_label_weights = label_weights[tail_indices]


            loss_cls_long_tail = self.loss_cls(
            tail_cls_score,
            tail_labels,
            tail_label_weights,
            avg_factor=class_weight[tail_labels].sum(),
            )
            
            num_tail = tail_indices.shape[0]
            total_queries = labels.shape[0]
            tail_ratio = num_tail / (total_queries + 1e-5)  # 防止除以零
           
            loss_cls = (1 - tail_ratio) * loss_cls + tail_ratio * loss_cls_long_tail
        ...
        unique_values = torch.unique(mask_targets)
    ...
    def forward(self, 
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            # print('len(cross_attention_list)=',len(cross_attention_list))
            if len(cross_attention_list)>=3:
                ...
                top_values, top_indices = torch.topk(max_values, k=topk[i-3])
                top_indices = top_indices.squeeze(0)
                # print('cross_attention=',cross_attention.shape)
                # print('max_values=',max_values.shape)
                # print('topk[i-1]=',topk[i-3])
                # print('top_indices=',top_indices.shape)
   
                selected, combined_indices = self.TailVoxelSampler(decoder_inputs[level_idx].permute(1,0,2),top_indices,M=topk[i-3])
            ...
            decoder_positional_encodings_attention = decoder_positional_encodings[level_idx]
            scene_embed_fov_attention = decoder_inputs[level_idx]




            # combined_indices = None
            # attn_masks_attention = attn_masks
            

            # query_feat,_ = layer(
            #         query=query_feat,
            #         key=scene_embed_fov_attention, #,decoder_inputs[level_idx],
            #         value=scene_embed_fov_attention, #decoder_inputs[level_idx],
            #         query_pos=query_embed,
            #         key_pos=decoder_positional_encodings_attention,
            #         attn_masks=attn_masks_attention,
            #         query_key_padding_mask=None,
            #         key_padding_mask=None)
            # print('decoder_inputs[level_idx]=',decoder_inputs[level_idx].shape)
            # print('scene_embed_fov_attention=',scene_embed_fov_attention.shape)
           

            # 检查所有元素是否为有效数值（非 NaN/非 Inf）
            # is_valid = torch.isfinite(query_feat).all().item()
            # if not is_valid:
            #     print("query_feat before 包含 NaN 或 Inf！")

            # attention_scores = query_feat.squeeze(1) @ scene_embed_fov_attention.squeeze(1).transpose(-2, -1)  # [batch, heads, q_len, k_len]
            # attention_probs = torch.softmax(attention_scores, dim=-1)

            # if torch.isnan(attention_probs).any():
            #     print("before !!! NaN出现在注意力权重中！检查Softmax输入:")
            #     print("Max attention_scores:", attention_scores.max())
            #     print("Min attention_scores:", attention_scores.min())
            #query_feat = torch.nn.LayerNorm(query_feat.shape[-1])(query_feat)
            #scene_embed_fov_attention = torch.nn.LayerNorm(scene_embed_fov_attention.shape[-1])(scene_embed_fov_attention)
...
#     def __init__(self, in_dim=192, num_classes=17, M=4096):
#         super().__init__()
#         self.M = M  # 需要采样的新体素数
...
#     def forward(self, voxel_feat, prev_indices,M, target_classes=[2,5,7,8,10,12,14]):
#         """
#         Args:
#             voxel_feat: [1, N, 128] 体素特征
#             prev_indices: [K] 已选体素索引
...
#         return selected, combined
    ...
    def sampling_target_voxels(self,
        target: torch.Tensor,
...
class HardVoxelMiningHead(nn.Module):
    ...
    def forward(self, voxel_feat, coarse_prediction, target, tail_classes=[2, 3, 4, 5, 6, 7, 8, 12, 14, 16, 18, 19],head_class=[1,9,10,11,13,15,17],empty_class=[0]):# [2, 3, 4, 5, 6, 7, 8, 12, 14, 16, 18, 19]
        """
        前向传播函数。
        Args:
