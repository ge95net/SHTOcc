...
def validate_submission(nusc: NuScenes,
    ...
    for sample_token in tqdm(sample_tokens, disable=not verbose):
        ...
        if len(nusc.lidarseg) > 0:  # If ground truth exists, compare the no. of predictions with that of ground truth.
            print('nusc.dataroot=',nusc.dataroot)
            print('sd_token=',sd_token)
            print('filename=',nusc.get('lidarseg', sd_token)['filename'])
            print('path=',os.path.join(nusc.dataroot, nusc.get('lidarseg', sd_token)['filename']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataroot', type=str, default='/mnt/cfs/algorithm/yunpeng.zhang/public_data/nuScenes',
