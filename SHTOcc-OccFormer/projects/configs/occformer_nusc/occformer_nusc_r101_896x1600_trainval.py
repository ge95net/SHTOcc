...
test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles_OccFormer', is_train=False,
            data_config=data_config, img_norm_cfg=img_norm_cfg),
    dict(type='LoadNuscOccupancyAnnotations', is_test_submit=True, is_train=False, 
            grid_size=occ_size, point_cloud_range=point_cloud_range, 
            bda_aug_conf=bda_aug_conf, cls_metas=nusc_class_metas),
    dict(type='OccDefaultFormatBundle3D', class_names=class_names, with_label=False), 
    dict(type='Collect3D', keys=['img_inputs', 'points_occ'],
            meta_keys=['pc_range', 'occ_size', 'sample_idx', 'timestamp', 'lidar_token',
...
test_config=dict(
    type=dataset_type,
    data_root=data_root,
    ann_file='data/nuscenes/nuscenes_infos_temporal_test.pkl',
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='data/nuscenes/nuscenes_infos_temporal_trainval.pkl',
