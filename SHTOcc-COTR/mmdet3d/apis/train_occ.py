...
from mmdet3d.core.hook import CustomDistEvalHook
...
def train_segmentor(model,
    ...
    logger.info("="*50)
    logger.info("Freezing parameters not containing 'cls_embed'")
    
    trainable_params = []
    for name, param in model.named_parameters():  # 注意：直接使用model而非model.module
    ...
    for name in trainable_params:
        logger.info(f"  {name}")
    logger.info(f"Total: {len(trainable_params)}")
    logger.info("="*50)
...
def train_detector(model,
    ...
    logger.info("="*50)
    logger.info("Freezing parameters not containing 'occ_predictor'")
    
    trainable_params = []
    for name, param in model.named_parameters():  # 注意：直接使用model而非model.module
        if "predictor"  in name or 'occ_predictor' in name:
            param.requires_grad = True
            trainable_params.append(name)
        else:
            param.requires_grad = False
