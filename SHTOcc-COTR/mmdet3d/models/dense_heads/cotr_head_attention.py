...
class COTRHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.refined_mlp = nn.Conv1d(embed_dims + self.num_classes+1, self.num_classes+1, 1)  # 细化预测的 MLP
        # not elegant

        # self.cams_embeds = nn.Parameter(
        #     torch.Tensor(6, self.embed_dims))
    ...
    def forward(self, img_feats, depth, img_metas, 
        ...
        occ_mask = torch.zeros((bs, int(occ_h), occ_w, occ_z),
        ...
        # print('occ_queries=',occ_queries.shape)
        # print('feat_flatten=',feat_flatten.shape)
        # attention_weights =  self.manual_attention_batch(occ_queries,feat_flatten)

        # cross_attention = attention_weights.permute(1,0).unsqueeze(0)
        # print('cross_attention=',cross_attention.shape)
        # max_values, _ = torch.max(cross_attention, dim=1)
        # print('max_values=',max_values.shape)
        # top_values, top_indices = torch.topk(max_values, k=2000)
        # top_indices = top_indices.squeeze(0)
        # print('top_indices=',top_indices.shape)
        # print('top_indices=',top_indices.shape)
        # selected, combined_indices = self.TailVoxelSampler(occ_queries.permute(1,0,2),top_indices,M=2000)
        # print('combined_indices=',combined_indices.shape)
        # occ_queries1 = occ_queries[:,:8000,:]
        # occ_pos1 = occ_pos[:,:,:10,:,:]
        ...
        #print_memory_usage('first encoder')
        ...
        #print('occ_cat=',occ_feature.shape)
        compact_occ = self.enhance_conv(occ_cat)
        #print('compact_occ=',compact_occ.shape)
        ...
        del occ1,occ0,occ2,enhanced_occ_feature,occ_cat,occ_0,occ_1,occ_queries,occ_pos
        # -----------------Encoder---------------------
        ...
        if self.training or not self.test_cfg.only_encoder:
            ...
            decoder_spatial_shapes = torch.tensor([
                                        [int(w*0.1), int(h), int(z)],
                                    ], device=object_query.device)
            lsi = torch.tensor([0,], device=object_query.device)

  
            attention_weights, output =  self.manual_attention(object_query,occ_value)
            ...
            selected, combined_indices = self.TailVoxelSampler(occ_value.permute(1,0,2),top_indices,M=2000)
            del attention_weights,cross_attention,max_values,top_indices
            #combined_indices = top_indices
            occ_value_attention = occ_value[combined_indices,:,:]
            ...
            #print_memory_usage('second decoder')
            #print('decoder_out=',decoder_out.shape)
            # -----------------Decoder---------------------
       
        # -----------------Predictor---------------------
        occ_outs1 = fullres_occ.permute(0, 2, 3, 4, 1) #bcwhz -> bwhzc

        occ_outs = self.occ_predictor(occ_outs1)
        #print('occ_outs 22=',occ_outs.shape)
    ...
    def loss(self,
        ...
        occ_outs_feature = preds_dicts['occ_outs_feature']

        hvm_out_dict = self.hvm_voxel(occ_outs_feature.permute(0,4,1,2,3),occ_outs.permute(0,4,1,2,3),voxel_semantics)
    ...
    def manual_attention(self, Q, V, K=None):
        """
        修正后的注意力计算函数，支持多头注意力，并对多头注意力结果取平均
        
        Args:
            Q (Tensor): Query 矩阵, shape [N, h, D] (e.g., [600, 2, 256])
            V (Tensor): Value 矩阵, shape [M, h, D] (e.g., [40000, 2, 256])
            K (Tensor, optional): Key 矩阵，如果未提供则 K = V
        """
        if K is None:
            K = V
        
        # 检查是否是单头情况
        if Q.dim() == 3 and Q.size(1) == 1:
            # 单头情况，移除中间的冗余维度
            Q = Q.squeeze(1)  # [N, D]
            K = K.squeeze(1)  # [M, D]
            V = V.squeeze(1)  # [M, D]
            ...
            # 3. 计算注意力权重
            attention_weights = F.softmax(scaled_scores, dim=-1)  # [N, M]
            
            # 4. 乘以 Value
            output = torch.matmul(attention_weights, V)  # [N, D]
        else:
            # 多头情况
            # 交换维度使头维度在前 [h, N, D] 和 [h, M, D]
            Q = Q.permute(1, 0, 2)  # [h, N, D]
            K = K.permute(1, 0, 2)  # [h, M, D]
            V = V.permute(1, 0, 2)  # [h, M, D]
            
            # 1. 计算点积 (使用广播机制)
            scores = torch.matmul(Q, K.permute(0, 2, 1))  # [h, N, M]
            ...
            # 3. 计算注意力权重
            attention_weights = F.softmax(scaled_scores, dim=-1)  # [h, N, M]
            
            # 4. 对多头注意力权重取平均
            mean_attention_weights = attention_weights.mean(dim=0)  # [N, M]
            
            # 5. 乘以 Value (需要将V转换为[M, h, D] -> [M, D]的均值)
            mean_V = V.permute(1, 0, 2).mean(dim=1)  # [M, D]
            output = torch.matmul(mean_attention_weights, mean_V)  # [N, D]
            
            # 将attention_weights形状调整为[N, M] (已经取平均)
            attention_weights = mean_attention_weights
        
        return attention_weights, output

    
    def TailVoxelSampler(self, voxel_feat, prev_indices,M, target_classes=[0,2,5,6,7,8,9]):
        """
        Args:
        ...
        logits = self.occ_predictor(voxel_feat[0])  # [N, num_classes]
        ...
        combined = torch.cat([prev_indices, selected])
        return selected, combined
    

    def manual_attention_batch(self,Q, V, K=None):
        if K is None:
            K = V
        
        # 移除中间的冗余维度（倒数第二个维度）
        Q = Q.squeeze(0)  # 原形状 [..., 1, D] → [..., D]
        K = K.squeeze(-2)
        V = V.squeeze(-2)
        ...
        print('V=',V.shape)
        # 计算点积（支持广播）
        scores = torch.matmul(Q, K.transpose(-2, -1))  # 结果形状 [B, N, M]
        ...
        # 注意力权重
        attention_weights = F.softmax(scaled_scores, dim=-1)  # [B, N, M]
        
        # 应用注意力到Value
        output = torch.matmul(attention_weights, V)  # [B, N, D]
        
        # 平均所有批次的结果
        avg_output = output.mean(dim=0)         # 形状 [N, D]
        avg_attention = attention_weights.mean(dim=0)  # 形状 [N, M]
    ...
    def hvm_voxel(self, voxel_feat, coarse_prediction, target_voxels, tail_classes=[0,2,5,6,7,8,9]):# [2, 3, 4, 5, 6, 7, 8, 12, 14, 16, 18, 19]
        """
        前向传播函数。
        Args:
...
def hvm_loss(pred, tag='coarse'):
    ...
    loss_dict['loss_voxel_hvm_{}'.format(tag)] = tail_class_loss
