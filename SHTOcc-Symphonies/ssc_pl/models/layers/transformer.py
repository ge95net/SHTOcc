...
class TransformerLayer(nn.Module):
    ...
    def forward(self, query, key=None, value=None, query_pos=None, key_pos=None):
        ...
        # q, attn_weights = self.attn(self.norm1(query), key, value)
   
        # max_values, _ = torch.max(attn_weights, dim=2)
     
        
        # top_values, top_indices = torch.topk(max_values, k=20)
...
class TransformerLayer_attention(nn.Module):
    ...
    def forward(self, query, key=None, value=None, query_pos=None, key_pos=None):
        ...
        if query_pos is not None:
            out_query,attention = self.attn(self.norm1(query) + query_pos, key, value)
            query  = query + out_query
        else:
            out_query,attention = self.attn(self.norm1(query), key, value)
            query  = query + out_query
