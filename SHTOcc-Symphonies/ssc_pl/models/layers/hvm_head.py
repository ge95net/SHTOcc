...
def sampling_hard_voxels(mask, N, t=3, omega=0.75):
    ...
    voxels_global_hardness_sampled = random_sampled_coords.view(-1, 3)[voxel_index.view(-1), :].view(B, int(omega * N), 3)
...
class HardVoxelMiningHead(nn.Module):
    def __init__(self, in_channel=532, embed_dims=128, num_classes=20):
        super().__init__()
        self.t = 3
        self.omega = 0.75
        self.N = 4096
