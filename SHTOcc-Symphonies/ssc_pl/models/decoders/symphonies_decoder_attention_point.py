...
from ..layers import (ASPP, DeformableSqueezeAttention, DeformableTransformerLayer,TransformerLayer_attention,
                      LearnableSqueezePositionalEncoding, TransformerLayer, Upsample,HardVoxelMiningHead,TailVoxelSampler)
...
class SymphoniesLayer(nn.Module):
    ...
    def forward(self,
        ...
        if attention != None:
            ...
            tail_indices, combined_indices = self.TailVoxelSampler(scene_embed_fov,top_indices,M=topk[i-1])
            #print('combined_indices=',combined_indices.shape)
            #combined_indices = top_indices
            scene_pos_fov = scene_pos_fov[:,combined_indices,:]
            scene_embed_fov_attention = scene_embed_fov[:,combined_indices,:]
            #tail_indices = None
        ...
        if combined_indices == None:
            ...
            ref_vox = ref_vox[:, fov_mask.squeeze()][:,combined_indices,:,:]
        
            scene_shape = torch.tensor([[1,1,topk[i-1]*2]],device=scene_embed_fov_attention.device)
            scene_level_index = get_level_start_index(scene_shape)
        ...
        if not self.query_update:
            if combined_indices != None:
                scene_embed_fov[:,combined_indices,:] = scene_embed_fov_attention
            else:
                scene_embed_fov = scene_embed_fov_attention

         
            scene_embed,coordinates1 = index_fov_back_to_voxels(scene_embed, scene_embed_fov, fov_mask,head_indices=top_indices,tail_indices=tail_indices)
            coords_first_4000 = coordinates1[:4000]  # (4000, 3)
            ...
            return scene_embed, inst_queries,attention,_
        ...
        if combined_indices != None:
            ...
            scene_embed_fov = scene_embed_fov_attention
        scene_embed,coordinates = index_fov_back_to_voxels(scene_embed, scene_embed_fov, fov_mask,head_indices=top_indices,tail_indices=tail_indices)

        return scene_embed, inst_queries,attention,coordinates
...
class SymphoniesDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.segmentation_head = nn.Conv3d(embed_dims, num_classes, kernel_size=1)
        #self.segmentation_head_512 = nn.Conv3d(512, 128, kernel_size=1)
        self.use_hvm = use_hvm
        if self.use_hvm:
            self.hvm_head = HardVoxelMiningHead(
                in_channel=embed_dims + num_classes, embed_dims=embed_dims, num_classes=num_classes
            )
    ...
    def forward(self, pred_insts, feats, pred_masks, depth, K, E, voxel_origin, projected_pix,
                fov_mask,target,text_features=None):
        ...
        scene_embed,_ = self.voxel_proposal(scene_embed, feats, scene_pos, vol_pts, ref_pix)
        ...
        for i, layer in enumerate(self.layers):
           ...
           #[int(self.num_queries*0.1*0.25*0.25), int(self.num_queries*0.1*0.25), int(self.num_queries*0.1)] 
            if i == len(self.layers):
                scene_embed, inst_queries,attention,_ = layer(scene_embed, inst_queries, feats, scene_pos, inst_pos,
                                              ref_2d, ref_3d, ref_vox, fov_mask,i,self.voxel_grid,topk=[4000,4000,16000],scene_shape=self.scene_shape,bs=bs,attention=attention)
            else:
                scene_embed, inst_queries,attention,coordinates = layer(scene_embed, inst_queries, feats, scene_pos, inst_pos,
                                              ref_2d, ref_3d, ref_vox, fov_mask,i,self.voxel_grid,topk=[4000,4000,16000],scene_shape=self.scene_shape,bs=bs,attention=attention)
            ...
            if i == 2:
                ...
                #print('self.cls_head1(scene_embed)=',self.cls_head1(scene_embed).shape)
          
                upsampled_scene_embed = self.upsample(scene_embed)
              
                corse_prediction = self.segmentation_head(upsampled_scene_embed)
                
                  
                outs.append(corse_prediction)
                if target!=None and self.use_hvm:
                    hvm_outs_list.append(upsampled_scene_embed)
        #allocated,reserved = self.print_memory_usage("decoder layer")  
        if target!=None and self.use_hvm:
            hvm_out_dict = self.hvm_head(hvm_outs_list[-1],corse_prediction,target)

 
            return outs, hvm_out_dict
        
        
        return outs,coordinates1
