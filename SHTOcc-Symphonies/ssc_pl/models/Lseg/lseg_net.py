...
class LSeg(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        print('self.text=',self.text.shape)
    def forward(self, x, labelset=''):
        ...
        image_features = self.scratch.output_conv(image_features.float().view(imshape[0], imshape[2], imshape[3], -1).permute(0,3,1,2))
        logits_per_image = self.scratch.output_conv(logits_per_image.float().view(imshape[0], imshape[2], imshape[3], -1).permute(0,3,1,2))


        # semantic_img = out[0]   
        # print('semantic_img=',semantic_img.shape)
        # image = semantic_img.permute(1,2,0).cpu().numpy()
        # image = image * 0.5 + 0.5
        ...
        # print(xxx)
        return out,logits_per_image,image_features
